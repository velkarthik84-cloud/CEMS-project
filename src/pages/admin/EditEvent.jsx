...
const EditEvent = () => {
  ...
  const fetchEvent = async () => {
    try {
      ...
      setValue('venue', eventData.venue || '');
      setValue('meetingLink', eventData.meetingLink || '');
      setValue('eventDate', formatDate(eventData.eventDate));
      setValue('startTime', eventData.startTime);
      setValue('endTime', eventData.endTime);
      setValue('registrationStart', formatDate(eventData.registrationStart));
      setValue('registrationEnd', formatDate(eventData.registrationEnd));

      setEventRoles(
        (eventData.eventRoles || []).map((r) => ({
          id: r.id || Date.now() + Math.random(),
          roleName: r.roleName || '',
          personName: r.personName || '',
          email: r.email || '',
      ...
      setBannerImage(eventData.bannerUrl || null);
    } catch (err) {
      console.error(err);
      toast.error('Failed to load event');
    } finally {
  ...
  const addRole = () =>
    setEventRoles([
      ...eventRoles,
      { id: Date.now(), roleName: '', personName: '', email: '', phone: '' },
    ]);

  const updateRole = (id, field, value) =>
    setEventRoles(
      eventRoles.map((r) => (r.id === id ? { ...r, [field]: value } : r))
    );

  const removeRole = (id) =>
    setEventRoles(eventRoles.filter((r) => r.id !== id));
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const roles = eventRoles.filter(
        (r) => r.roleName && r.personName
      );
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card>
          <h2 className="section-title">
            <Calendar className="icon" /> Basic Information
          </h2>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <Select label="Type" options={EVENT_TYPES} {...register('type')} />
          </div>
        </Card>
        ...
        <Card>
          <h2 className="section-title">
            ...
            {watchType === 'online' ? 'Meeting' : 'Venue'}
          </h2>

          {watchType === 'online' ? (
            <Input label="Meeting Link" {...register('meetingLink')} />
          ) : (
            <Textarea label="Venue Address" rows={3} {...register('venue')} />
          )}
        </Card>
        ...
        <Card>
          <h2 className="section-title">
            <Clock className="icon" /> Schedule
          </h2>

          <div className="grid md:grid-cols-3 gap-4">
            <Input type="date" label="Date" {...register('eventDate')} />
            <Input type="time" label="Start" {...register('startTime')} />
            <Input type="time" label="End" {...register('endTime')} />
          </div>

          <div className="grid md:grid-cols-2 gap-4 mt-4">
            <Input type="date" label="Registration Opens" {...register('registrationStart')} />
            <Input type="date" label="Registration Closes" {...register('registrationEnd')} />
          </div>
        </Card>
        ...
        <Card>
          <div className="flex justify-between mb-4">
            <h2 className="section-title"><Users className="icon" /> Event Roles</h2>
            <Button size="sm" icon={Plus} onClick={addRole}>Add Role</Button>
          </div>

          {eventRoles.map((r, i) => (
            <div key={r.id} className="p-4 border rounded-xl mb-3 bg-gray-50">
              <div className="flex justify-between mb-2">
                <p className="font-semibold">Role {i + 1}</p>
              ...
              <div className="grid md:grid-cols-2 gap-3">
                <Input value={r.roleName} placeholder="Role" onChange={(e) => updateRole(r.id, 'roleName', e.target.value)} />
                <Input value={r.personName} placeholder="Name" onChange={(e) => updateRole(r.id, 'personName', e.target.value)} />
                <Input value={r.email} placeholder="Email" onChange={(e) => updateRole(r.id, 'email', e.target.value)} />
                <Input value={r.phone} placeholder="Phone" onChange={(e) => updateRole(r.id, 'phone', e.target.value)} />
              </div>
            </div>
          ))}
        </Card>
        ...
        <Card>
          <h2 className="section-title">
            <ImageIcon className="icon" /> Banner Image
          </h2>
          <div className="p-4 bg-gray-50 border-dashed border rounded-xl">
            <FileUpload accept="image/*" value={bannerImage} onChange={setBannerImage} />
          </div>
        </Card>
